...
int removerNodoCaso1e2(Nodo** raiz, Nodo* no) {
	...
	if(no->pai) {
		...
		if(!no->direita && !no->esquerda) {
			...
			//se o nodo for o filho da esquerda
...
void removerNodo(Nodo** no, int chave) {

	//busca nodo na arvore
	Nodo* encontrado = buscarNodo(*no, chave);

	//caso retorne um nodo encontrado
	if(encontrado) {

		//tenta remover para o caso 1 e 2
		if(!removerNodoCaso1e2(no, encontrado)) {
			...
			removerNodoCaso1e2(no, sucessor);
		}
